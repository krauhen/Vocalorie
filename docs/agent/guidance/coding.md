...
7. **Prefer a value type over three or more same-typed positional parameters — Primitive Obsession (Fowler, Code Smells).** Eight `String`s in a row compile happily when two are transposed. Wrap them (see `EditableNutrition`) so the compiler catches it.
