...
Capture the intent of the actionable tips shown beneath the day nutrition score: a ranked, rule-derived read-out of the score's own decomposition, so the user learns which shortfall costs the most points today and what to do about it. Wording may optionally be reworded by the LLM on explicit request, never generated by it.
