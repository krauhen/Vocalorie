...
- [ ] 4.7 Confirm the backlog reflects the promotion — the F4 file is removed from `docs/agent/backlog/features/` and `docs/agent/backlog/features/README.md` lists it under `## Promoted` pointing at this change. Done when the proposal was written; no edit expected. Its investigation is in git history if the file:line current state is needed.
