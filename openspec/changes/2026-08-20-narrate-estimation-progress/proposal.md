...
- **The estimation seam gains a progress channel.** `NutritionEstimator.estimate(...)` (`ai/KoogNutritionAgent.kt:41-48`) takes an optional progress callback with a no-op default, so every existing call site and every fake in `src/test` keeps compiling unchanged.
...
- **Koog's `EventHandler` feature supplies the remaining steps, behind a spike.** Its existence is documented but its exact 1.0.0 member names are unverified (design D6), so task group 1 confirms them against the resolved jar before any code depends on them. If they differ or are absent, the `onUrlFetched` path still delivers the feature with fewer steps.
- **The state holder gains one narrow in-flight field** beside `isLoading` (`ui/capture/MealCaptureUiState.kt:57`), modelled on `tipsRewordingInFlight` (`:74`), set from the callback inside `runEstimate` (`ui/capture/MealCaptureViewModel.kt:277-316`).
