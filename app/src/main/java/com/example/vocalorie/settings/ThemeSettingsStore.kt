...
class ThemeSettingsStore(context: Context) {
    ...
    /**
     * The nutrition targets moved to their own store; this store keeps forwarding to it only until
     * `ui/MealCaptureScreen.kt` and `data/repository/ThemeSettingsRepository.kt` take
     * [NutritionSettingsStore] directly. Both stores read the same preference file, so which one a
     * caller holds makes no difference to what is stored.
     */
    private val nutritionSettings = NutritionSettingsStore(context)
    ...
    fun savePrimary(color: Color) {
        saveColor(KEY_PRIMARY, color)
    }

    @Synchronized
    fun saveSecondary(color: Color) {
        saveColor(KEY_SECONDARY, color)
    }

    @Synchronized
    fun saveAccent(color: Color) {
        saveColor(KEY_ACCENT, color)
    }

    @Synchronized
    fun saveBackground(color: Color) {
        saveColor(KEY_BACKGROUND, color)
    }

    @Synchronized
    fun saveSurface(color: Color) {
        saveColor(KEY_SURFACE, color)
    }

    @Synchronized
    fun saveSurfaceVariant(color: Color) {
        saveColor(KEY_SURFACE_VARIANT, color)
    }
    ...
    // --- Nutrition targets: forwarded to [NutritionSettingsStore], which owns them. ---

    fun getBaseCaloriesBurned(): Int = nutritionSettings.getBaseCaloriesBurned()

    fun saveBaseCaloriesBurned(value: Int) = nutritionSettings.saveBaseCaloriesBurned(value)

    fun getKcalPerStep(): Double = nutritionSettings.getKcalPerStep()

    fun saveKcalPerStep(value: Double) = nutritionSettings.saveKcalPerStep(value)

    fun getNutritionGoals(): NutritionGoals = nutritionSettings.getNutritionGoals()

    fun saveNutritionGoals(goals: NutritionGoals) = nutritionSettings.saveNutritionGoals(goals)
    ...
    companion object {
        ...
        /** Forwarded so callers of the old location keep working; owned by [NutritionSettingsStore]. */
        internal fun cleanFloatPrecision(value: Float): Double = NutritionSettingsStore.cleanFloatPrecision(value)
    }
